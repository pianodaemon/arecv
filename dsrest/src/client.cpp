...
{
    int rc = REST_HTTP_CLIENT_SUCCEED;

    if( sr_ptr == nullptr )
    {
        rc = REST_HTTP_CLIENT_ARG_NULL_PTR;
    }
    else
    {
        CURL*  curl = nullptr;
        ...
        if ( curl == nullptr )
        {
            rc = REST_HTTP_CLIENT_CURL_FUCK_US;
        }
        else
        {
            ...
            if ( curl_rc == CURLE_OK )
            {
                *sr_ptr = new server_reply;
                ...
                if( *sr_ptr == nullptr )
                {
                    rc = REST_HTTP_CLIENT_SYSTEM_ERROR;
                }
                else
                {
                    ...
                    if( chunk.length > 0 )
                    {
                        ...
                        else rc = REST_HTTP_CLIENT_SYSTEM_ERROR;
                    }
                }
